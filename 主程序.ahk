...
一键跳转路径:
    ...
    $WinID := WinExist("A")
    ;MsgBox,1
    Firstpath:=Trim(Explorer_Path() "`n" DirectoryOpus_path2("Clipboard SET {sourcepath}") "`n" TotalCommander_path() "`n" XYplorer_Path("1") "`n" DoubleCommander_path(给dc发送热键),"`n")
        ...
        if WinActive(winTitle){
            if (全局变量11="0"){
                ...
                If (自动跳转到文件管理器路径 = "开启"){
                    ...
                    Firstpath22:=Trim(Explorer_Path() "`n" DirectoryOpus_path2("Clipboard SET {sourcepath}") "`n" TotalCommander_path() "`n" XYplorer_Path("1") "`n" DoubleCommander_path(给dc发送热键),"`n")
                    FolderPath222 :=""
                    Loop, parse, Firstpath22, `n, `r
                    {
                        if !(RegExMatch(A_LoopField, "^\s*$")){ ;判断是否是空白行
                            FolderPath222:=RTrim( RegExReplace(A_LoopField, "^\((.*?)\)"),"\") ;去掉括号和别名，并去掉结尾的斜杠
                            Break
                        }
                    }
